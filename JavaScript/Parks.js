...
$.ajax({
    ...
    for (var i = 0; i <= data.length; i++) {
      ...
      if(data[i]){
        ...
        createMarkerParks(data[i]);
        parkMarker.push(parmarker);
        markers.push(parmarker);
        markers[markers.length - 1].data = data[i];
        markers[markers.length - 1].type = "park";
	  }}
    }         
});
}
...
function toogleParkMarkers() {
  for (var i = 0; i < parkMarker.length; i++) {
    if(parkMarker[i].getVisible()) 
      parkMarker[i].setVisible(false);
    else 
      parkMarker[i].setVisible(true);
  }
}

var prev_infowindow ;

function createInfoPark(parmarker, data) {
	
	var infowindow = new google.maps.InfoWindow({
                       content: ""
                   });

	google.maps.event.addListener(parmarker, 'click', function() {
	  ...
	  prev_infowindow = infowindow;
		...
		infowindow.setContent(data._primary_decsription);
    infowindow.open(parmarker.get("map"), parmarker);
...
function createMarkerParks(data){

      parmarker = new google.maps.Marker({
      position: new google.maps.LatLng(data.location.coordinates[1],data.location.coordinates[0]),
      map: map,
      title: data.primary_type ,
      icon : 'Images/parkIcon.png'
      });
      createInfoPark(parmarker, data);
}


 function alertPark(){
  alert( "Showing " + parkMarker.length + " Parks! \nParks managed by the Chicago Park District.");
 }